...
Page({
  ...
  onShow:function() {
    ...
    db.collection('Products').where({
      ...
      success: res=> {
        ...
        complete: res=>{
          console.log("想看看222   "+this.data.goodsObj._id)
          // this.getGoodsDetail(goods_id);
          let collect=wx.getStorageSync("collect")||[];
          // 2判断商品是否被收藏
          console.log("收藏了吗    "+isCollect);
  ...
  //   })
   ...
   getGoodsDetail(goods_id){
    // const goodsList=wx.getStorageSync("allgoods");
    // const goodsList=this.data.goods_info;
    // this.setData({
    //   goodsObj:{
    //     goods_name: goodsList[0].name,
    //     goods_price: goodsList[0].price,
    //     goods_introduce:goodsList[0].description,
    //     // goods_introduce: res.data.message.goods_introduce.replace(/\.webp/g,'.jpg'),
    //     pics: goodsList[0].image,
    //     sold_out:goodsList[0].sold_out_or_not
    //     },
    //   goods_id:goodsList[0]._id,
    //   urls:goodsList[0].image
    // })
    // for(var i=0;i<goodsList.length;i++){
    //   if(goodsList[i]._id===goods_id){
    //     this.GoodsInfo=goodsList[i];
  ...
  handleCollect(){
    ...
    const db = wx.cloud.database();
    let collect=wx.getStorageSync("collect")||[];
    let index=collect.findIndex(v=>v._id===this.data.goodsObj._id);
    if(index!==-1){
      // 已经收藏
      collect.splice(index,1);
      isCollect=false;
